...
# Profiling lived in nuPlan's training package.  Keep it optional for the
# runtime-only companion so importing NuBoard does not require training code.
try:
    ...
    ProfileCallback = None  # type: ignore[assignment,misc]
    _PROFILE_AVAILABLE = False

logger = logging.getLogger(__name__)


def _default_websocket_origins(port_number: int) -> List[str]:
    """Return the loopback origins accepted by a local NuBoard server."""
    return [f"localhost:{port_number}", f"127.0.0.1:{port_number}"]


def _normalise_websocket_origins(
    origins: Optional[Iterable[str]], port_number: int
) -> List[str]:
    """Normalise an explicit origin allowlist and reject wildcard access."""
    if origins is None:
        return _default_websocket_origins(port_number)
    if isinstance(origins, str):
        origins = origins.split(",")
    normalised = [str(origin).strip() for origin in origins if str(origin).strip()]
    if not normalised:
        return _default_websocket_origins(port_number)
    if "*" in normalised:
        raise ValueError(
            "NuBoard websocket_origins must be an explicit host allowlist; '*' is not supported"
        )
    return normalised
...
class NuBoard:
    ...
    def __init__(
        ...
        websocket_origins: Optional[Iterable[str]] = None,
    ):
        """
        Nuboard main class.
        ...
        :param address: Interface on which Bokeh listens. Defaults to loopback.
        :param websocket_origins: Explicit browser Origin allowlist. When omitted,
            only localhost and 127.0.0.1 on the configured port are accepted.
        ...
        if not self._address:
            raise ValueError("NuBoard address must not be empty")
        self._websocket_origins = _normalise_websocket_origins(websocket_origins, port_number)
    ...
    def run(self) -> None:
        """Run nuBoard WebApp."""
        logger.info(f"Opening Bokeh application on http://{self._address}:{self._port_number}/")
        logger.info(f"Allowed websocket origins: {self._websocket_origins}")
        ...
        if self._profiler_path is not None:
            ...
            if not _PROFILE_AVAILABLE:
                raise ImportError(
                    "NuBoard profiling requires the optional nuPlan training extras; "
                    "unset profiler_path for the public runtime companion."
                )
        ...
        server = Server(
            ...
            allow_websocket_origin=self._websocket_origins,
