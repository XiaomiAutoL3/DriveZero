...
@dataclass
class ModelInputPlot(BaseScenarioPlot):
    """A dataclass for DriveRL model-input plot."""

    data_sources: Dict[int, Dict[str, ColumnDataSource]] = field(default_factory=dict)
    lane_plot: Optional[GlyphRenderer] = None
    curb_plot: Optional[GlyphRenderer] = None
    ego_plot: Optional[GlyphRenderer] = None
    agent_plot: Optional[GlyphRenderer] = None
    ...
    def update_plot(self, main_figure: Figure, frame_index: int, doc: Document) -> None:
        ...
        with self.data_source_condition:
            ...
            data_sources = self.data_sources[frame_index]

            def update_main_figure() -> None:
                lanes = dict(data_sources["lanes"].data)
                curbs = dict(data_sources["curbs"].data)
                ego = dict(data_sources["ego"].data)
                agents = dict(data_sources["agents"].data)
                if self.lane_plot is None:
                    self.lane_plot = main_figure.multi_line(
                        xs="xs",
                        ys="ys",
                        line_color="#f59e0b",
                        line_width=2.0,
                    ...
                    self.curb_plot = main_figure.multi_line(
                        xs="xs",
                        ys="ys",
                        line_color="#dc2626",
                        line_width=3.0,
                    ...
                    self.ego_plot = main_figure.multi_polygons(
                        xs="xs",
                        ys="ys",
                        fill_color="#22c55e",
                        fill_alpha=0.35,
                    ...
                    self.agent_plot = main_figure.multi_polygons(
                        xs="xs",
                        ys="ys",
                        fill_color="#38bdf8",
                        fill_alpha=0.25,
                    ...
                    self.lane_plot.data_source.data = lanes
                    self.curb_plot.data_source.data = curbs  # type: ignore[union-attr]
                    self.ego_plot.data_source.data = ego
                    self.agent_plot.data_source.data = agents
    ...
    def update_data_sources(self, history: SimulationHistory) -> None:
        """Update DriveRL model-input data sources."""
        if not self.data_source_condition:
            return

        with self.data_source_condition:
            for frame_index, sample in enumerate(history.data):
                debug_info = getattr(sample.trajectory, "debug_info", {}) or {}
                model_input = debug_info.get("model_input") or {}
                road_graph = model_input.get("road_graph") or {}
                lanes = road_graph.get("lanes") or []
                lane_types_are_raw = bool(road_graph.get("lane_types_are_raw", False))
                ego = model_input.get("ego") or {}
                other_agents = model_input.get("other_agents") or []
                ...
                for lane in lanes:
                    ...
                    is_curb = lane.get("is_curb")
                    if is_curb is None:
                        curb_type = 0 if lane_types_are_raw else 1
                        is_curb = int(lane.get("lane_type", -1)) == curb_type
                    if is_curb:
                        curb_xs.append(xs)
                        curb_ys.append(ys)
                    else:
                ...
                ego_polygons = [_to_bokeh_polygon(ego.get("polygon"))] if ego.get("polygon") else []
                agent_polygons = [
                    _to_bokeh_polygon(agent.get("polygon"))
                    for agent in other_agents
                    if agent.get("polygon")
                ]

                self.data_sources[frame_index] = {
                    "lanes": ColumnDataSource(dict(xs=lane_xs, ys=lane_ys)),
                    "curbs": ColumnDataSource(dict(xs=curb_xs, ys=curb_ys)),
                    "ego": ColumnDataSource(
                        dict(xs=[poly[0] for poly in ego_polygons], ys=[poly[1] for poly in ego_polygons])
                    ),
                    "agents": ColumnDataSource(
                        dict(xs=[poly[0] for poly in agent_polygons], ys=[poly[1] for poly in agent_polygons])
                    ),
                }
                self.data_source_condition.notify()


def _to_bokeh_polygon(points: List[List[float]]) -> tuple[List[List[List[float]]], List[List[List[float]]]]:
    xs = [float(point[0]) for point in points]
    ys = [float(point[1]) for point in points]
    return [[xs]], [[ys]]
...
class SimulationFigure:
    ...
    model_input_plot: Optional[ModelInputPlot] = None  # DriveRL model-input plot
    ...
    def __post_init__(self) -> None:
        ...
        if self.model_input_plot is None:
            self.model_input_plot = ModelInputPlot()
    ...
    def copy_datasources(self, other: SimulationFigure) -> None:
        ...
        self.model_input_plot.data_sources = other.model_input_plot.data_sources  # type: ignore
    ...
    def get_glyph_name_from_checkbox_group(self, glyph_checkbox_group_name: str) -> str:
        ...
        if not self.glyph_names_from_checkbox_group:
            self.glyph_names_from_checkbox_group = {
                ...
                'Drivable Area': SemanticMapLayer.DRIVABLE_AREA.name,
    ...
    def _get_trajectory_glyph_to_update(self, glyph_name: str) -> List[Optional[GlyphRenderer]]:
        ...
        elif glyph_name == 'Model Input':
            if self.model_input_plot is None:
                return []
            return [
                self.model_input_plot.lane_plot,
                self.model_input_plot.curb_plot,
                self.model_input_plot.ego_plot,
                self.model_input_plot.agent_plot,
            ]
    ...
    def update_glyphs_visibility(self, glyph_names: Optional[List[str]] = None) -> None:
        ...
        for glyph_name in glyph_names:
            ...
            elif glyph_name in ['Expert Trajectory', 'Ego Trajectory', 'Model Input', 'Goal', 'Traffic Light']:
    ...
    def update_legend(self) -> None:
        ...
        selected_map_polygon_layers = [
            SemanticMapLayer.DRIVABLE_AREA.name,
