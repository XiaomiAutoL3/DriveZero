...
class ScenarioTab(BaseTab):
    ...
    def __init__(
        ...
        trajectories = ['Expert Trajectory', 'Ego Trajectory', 'Model Input', 'Goal', 'Traffic Light', 'RoadBlock']
        self._traj_checkbox_group = CheckboxGroup(
            labels=trajectories,
            active=[index for index, label in enumerate(trajectories) if label != 'Model Input'],
    ...
    def _init_selection(self) -> None:
        ...
        if self.experiment_file_data.simulation_scenario_keys:
            first_key = self.experiment_file_data.simulation_scenario_keys[0]
            self._scenario_token_multi_choice.value = [first_key.scenario_name]
            self._scalar_scenario_type_select.value = first_key.scenario_type
            available_log_names = self.load_log_name(scenario_type=first_key.scenario_type)
            self._scalar_log_name_select.options = [""] + available_log_names
            self._scalar_log_name_select.value = first_key.log_name
            available_scenario_names = self.load_scenario_names(
                scenario_type=first_key.scenario_type,
                log_name=first_key.log_name,
            )
            self._scalar_scenario_name_select.options = [""] + available_scenario_names
            self._scalar_scenario_name_select.value = first_key.scenario_name
        self._update_planner_names()
