...
def _configured_websocket_origins(cfg: DictConfig) -> Optional[List[str]]:
    """Resolve an explicit browser Origin allowlist from env or Hydra config."""
    env_value = os.getenv("NUPLAN_NUBOARD_WEBSOCKET_ORIGINS")
    if env_value is not None:
        return [origin.strip() for origin in env_value.split(",") if origin.strip()]
    configured = getattr(cfg, "websocket_origins", None)
...
def initialize_nuboard(cfg: DictConfig) -> NuBoard:
    ...
    nuboard = NuBoard(
        ...
        address=os.getenv("NUPLAN_NUBOARD_ADDRESS", str(getattr(cfg, "address", "127.0.0.1"))),
        websocket_origins=_configured_websocket_origins(cfg),
