...
# Keep defaults relocatable for a source checkout and avoid writing into a
# user's home directory merely by importing the simulation entry point.  The
# evaluator still accepts explicit ``NUPLAN_DATA_ROOT``/``NUPLAN_EXP_ROOT``
# values for a prepared dataset and persistent output location.
DEFAULT_DATA_ROOT = 'data'
DEFAULT_EXP_ROOT = 'output'
...
def set_up_common_builder(cfg: DictConfig, profiler_name: str) -> CommonBuilder:
    ...
    if cfg.enable_profiling:
        # Profiling is optional and belongs to the removed training package.
        # Import it only when explicitly requested so ordinary simulation does
        # not require the private training tree.
        from nuplan.planning.training.callbacks.profile_callback import ProfileCallback
