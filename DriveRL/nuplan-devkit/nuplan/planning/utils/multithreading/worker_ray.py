...
def _ray_local_dashboard_options() -> tuple[bool, str]:
    """Return the local Ray dashboard policy without exposing it by default.

    Local evaluation does not need a dashboard. Keep it disabled unless a
    caller explicitly opts in, and bind an opted-in dashboard to loopback by
    default so it is not reachable from the worker network.
    """
    include_raw = os.getenv(
        'NUPLAN_RAY_INCLUDE_DASHBOARD',
        os.getenv('DRIVERL_EVAL_RAY_INCLUDE_DASHBOARD', '0'),
    ).strip().lower()
    if include_raw not in {'0', '1', 'false', 'true', 'no', 'yes'}:
        raise ValueError('NUPLAN_RAY_INCLUDE_DASHBOARD must be 0/1 or false/true')
    include_dashboard = include_raw in {'1', 'true', 'yes'}
    dashboard_host = os.getenv(
        'NUPLAN_RAY_DASHBOARD_HOST',
        os.getenv('DRIVERL_EVAL_RAY_DASHBOARD_HOST', '127.0.0.1'),
    ).strip()
    if not dashboard_host:
        raise ValueError('NUPLAN_RAY_DASHBOARD_HOST must not be empty')
    return include_dashboard, dashboard_host
...
def initialize_ray(
    ...
    if master_node_ip and use_distributed:
        ...
        # Never write the cluster credential to logs; task logs are often
        # collected or shared outside the evaluation host.
        ...
        include_dashboard, dashboard_host = _ray_local_dashboard_options()
        ray.init(
