...
def _ego_has_route(scenario: NuPlanScenario, map_radius: float) -> bool:
    ...
    proximal_map_objects = scenario.map_api.get_proximal_map_objects(
        ego_coords, map_radius, layer_names
    )
    map_lane_roadblock_ids = {
        str(map_object.get_roadblock_id())
        for layer_name in layer_names
        for map_object in proximal_map_objects.get(layer_name, [])
    }
    route_roadblock_ids = {str(identifier) for identifier in scenario.get_route_roadblock_ids()}
    return bool(map_lane_roadblock_ids.intersection(route_roadblock_ids))
