...
class NuPlanMap(AbstractMap):
    ...
    def __init__(self, maps_db: IMapsDB, map_name: str) -> None:
        ...
        self._map_object_getter: Dict[SemanticMapLayer, Callable[[str], MapObject]] = {
            ...
            SemanticMapLayer.DRIVABLE_AREA: self._get_drivable_area,
    ...
    def _get_drivable_area(self, drivable_area_id: str) -> NuPlanPolygonMapObject:
        """
        Gets the drivable area with the given drivable_area_id.
        :param drivable_area_id: desired unique id of a drivable area.
        :return: NuPlanPolygonMapObject object.
        """
        return (
            NuPlanPolygonMapObject(drivable_area_id, self._get_vector_map_layer(SemanticMapLayer.DRIVABLE_AREA))
            if drivable_area_id in self._get_vector_map_layer(SemanticMapLayer.DRIVABLE_AREA)["fid"].tolist()
            else None
        )
